...
for p in ivalues(PlayerNumber) do
	...
	if GAMESTATE:IsHumanPlayer(p) then
		t[#t+1] =  Def.ActorFrame{
			InitCommand=function(self)
			  ...
			  Def.Quad {
				...
				OnCommand=function(self) self:zoomto(336,0):easeoutsine(0.3):zoomto(336,61) end;
				OffCommand=function(self) self:sleep(0.175):easeoutsine(0.4):addy(-105) end;
			 };
			 -- Avatar and name
			Def.ActorFrame {
				OnCommand=function(self) 
					self:diffusealpha(0):sleep(0.3):easeoutsine(0.2):diffusealpha(1) 
				end;
				Def.Sprite {
		...
		t[#t+1] = Def.ActorFrame{
			...
			Def.ActorFrame {
				OnCommand=function(self) 
					self:diffusealpha(0):sleep(0.3):easeoutsine(0.3):diffusealpha(1) 
				end;
				Def.BitmapText {
...
if not GAMESTATE:IsCourseMode() then
	t[#t+1] = Def.ActorFrame {
		LoadActor(THEME:GetPathG("ScreenEvaluation", "StageDisplay")) .. {
			...
			OnCommand=function(self) self:diffusealpha(0):sleep(0.3):easeoutsine(0.3):diffusealpha(1) end;
			OffCommand=function(self) self:sleep(0.175):easeoutsine(0.4):diffusealpha(0) end;
		}
	}
else
	t[#t+1] =  Def.ActorFrame {
