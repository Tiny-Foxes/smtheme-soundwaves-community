return Def.ActorFrame {
	...
	local hiddenindicator = 0
		if song then
			if song:IsLong() then
				self:easeoutsine(0.15):diffusealpha(1)
			...
			elseif song:IsMarathon() then
				...
				if hiddenindicator ~= 1 then
					hiddenindicator = 1
					self:easeoutsine(0.15):diffusealpha(0)
				end;
			end
		else
			self:easeoutsine(0.15):diffusealpha(0)
	...
	OffCommand=function(self) self:stoptweening():easeoutsine(0.3):diffusealpha(0) end;
	LoadActor(THEME:GetPathB("_frame","3x1"),"rounded light", 160-16) .. {
		InitCommand=function(self) self:diffuse(color("#666666")) end;
		SetLongSongMessageCommand=function(self) self:easeoutsine(0.08):diffuse(color("#29739b")) end;
		SetMarathonSongMessageCommand=function(self) self:easeoutsine(0.08):diffuse(color("#29419b")) end;
	};
	Def.BitmapText {
		Font="_Semibold",
